...
- 応募時に入力したメールアドレスで「新規登録」し、パスワードを設定してログインします
- 初回ログイン時に、そのメールアドレスと一致する応募データへ自動的に紐付けられます（`members.email` 列で照合）
- 既存の会員（このマイページ機能を追加する前に応募された方）は、`contact` 欄からベストエフォートでメールアドレスを抽出済みですが、うまく抽出できていない場合は管理者が `members` テーブルの `email` 列を手動で設定してください
